...
export default function Skills() {
  ...
  return (
    <section id="skills" className="py-20 bg-transparent border-b border-slate-900 transition-colors duration-300">
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 mb-12">
          {categories.map((cat) => (
            <button
              ...
              className={`px-5 py-2.5 rounded-full text-xs font-mono uppercase tracking-wider font-bold transition-all cursor-pointer border transition-colors duration-300 ${
                activeCategory === cat.value
                  ? 'bg-sky-600 text-white border-sky-600 shadow-lg'
                  : 'bg-slate-900 text-slate-400 border-slate-800 hover:border-slate-700 hover:text-white shadow-md'
              }`}
            >
              {cat.label}
            </button>
            ...
            return (
              ...
              >
                <ThreeDCard className="w-full h-full">
                  <div className="bg-slate-900 border border-slate-850 rounded-3xl p-6 md:p-8 hover:border-slate-800 shadow-md flex flex-col justify-between transition-colors duration-300 w-full h-full">
                    <div>
                      {/* Skill Header */}
                      <div className="flex items-center space-x-3.5 mb-5">
                        <div className="w-11 h-11 border border-slate-800 bg-slate-950 flex items-center justify-center text-sky-400 shrink-0 rounded-full transition-colors duration-300">
                          <Icon className="w-5 h-5" />
                        </div>
                        <div>
                          <h3 className="font-bold text-white text-base font-sans transition-colors duration-300">{skill.name}</h3>
                          <span className="inline-block mt-1 text-[9px] font-mono font-bold uppercase tracking-widest text-slate-400 border border-slate-800 bg-slate-950 px-2.5 py-0.5 rounded-full transition-colors duration-300">
                            {skill.category}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="mb-6">
                        <div className="flex items-center justify-between text-[10px] font-mono font-bold text-slate-500 mb-2 transition-colors duration-300">
                          <span>PROFICIENCY LEVEL</span>
                          <span className="text-slate-300">{skill.level}%</span>
                        </div>
                        <div className="w-full h-1.5 bg-slate-950 border border-slate-800 rounded-full overflow-hidden transition-colors duration-300">
                          <motion.div
                            initial={{ width: 0 }}
                      ...
                      {/* Description text */}
                      <p className="text-xs md:text-sm text-slate-400 leading-relaxed mb-6 font-sans transition-colors duration-300">
                        {skill.desc}
                      </p>
                    </div>

                    {/* Sub Bullet details */}
                    <div className="border-t border-slate-800 pt-5 mt-auto transition-colors duration-300">
                      <h4 className="text-[10px] font-mono uppercase tracking-widest text-slate-500 mb-3 transition-colors duration-300">CORE COMPETENCY FOCUS:</h4>
                      <ul className="space-y-2">
                        {skill.details.map((detail, dIdx) => (
                          <li key={dIdx} className="flex items-center space-x-2 text-xs text-slate-300 font-sans transition-colors duration-300">
